...
async function basicAgentInteraction() {
  ...
  const response = await financialAgent.generate('What are my recent transactions?');
...
async function agentWithOptions() {
  ...
  const response = await financialAgent.generate('Summarize my financial data', {
    modelSettings: {
      temperature: 0.7,
      maxOutputTokens: 500,
    },
...
async function conversationWithMemory() {
  ...
  // First message
  const response1 = await financialAgent.generate('Show me my largest transaction', {
    memory: {
  ...
  const response2 = await financialAgent.generate('What was the category of that transaction?', {
    memory: {
      thread: 'user-123-thread',
      resource: 'user-123',
    },
...
async function multipleAgents() {
  ...
  // Financial agent
  const financialResponse = await financialAgent.generate('How much did I spend on Amazon?');
  console.log('Financial Agent:', financialResponse.text);
  
  // Weather agent
  const weatherResponse = await weatherAgent.generate('What\'s the weather like in New York?');
...
async function structuredOutputExample() {
  ...
  const response = await financialAgent.generate('Analyze my spending and categorize it', {
    structuredOutput: {
...
async function toolControlExample() {
  ...
  // Disable tool usage
  const response2 = await financialAgent.generate('Just give me financial advice', {
    toolChoice: 'none',
  });
